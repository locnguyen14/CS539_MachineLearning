...
def Terms_and_Conditions():
    ...
    Read_and_Agree = (
        True
    )  # if you have read and agree with the term above, change "False" to "True".
    # *******************************************
...
def compute_Phi(x, p):
    ...
    Phi_row = x.shape[0]
    Phi_col = p
    Phi = np.asmatrix(np.zeros(shape=(Phi_row, Phi_col)))

    # Fill the matrix
    for i in range(Phi_row):
        for j in range(Phi_col):
            Phi[i, j] = np.power(x.item((i, 0)), j)
    #########################################
    return Phi
...
def least_square(Phi, y):
    ...
    # INSERT YOUR CODE HERE
    w = np.matmul(
        np.matmul(np.linalg.inv(np.matmul(Phi.transpose(), Phi)), Phi.transpose()), y
    )
...
def ridge_regression(Phi, y, alpha=0.001):
    ...
    #########################################
    # INSERT YOUR CODE HERE
    # Create an identity matrix, which is essential for the closed-form solution
    Phi_col = Phi.shape[1]
    Id_matrix = np.identity(Phi_col)

    w = np.matmul(
        np.matmul(
            np.linalg.inv(np.matmul(Phi.transpose(), Phi) + alpha * Id_matrix),
            Phi.transpose(),
        ),
        y,
    )
