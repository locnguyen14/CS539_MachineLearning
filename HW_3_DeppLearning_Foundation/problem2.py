...
def compute_a(z):
    ...
    #########################################
    # INSERT YOUR CODE HERE
    # Compute the bound for z that don't encounter overflow or underflow problem
    # Using np.log(np.finfo(type(z)).max), we determine the max number python can take exp is 709
    z_bound = 709.0
...
def compute_L(a, y):
    ...
    #########################################
    # INSERT YOUR CODE HERE
    if (a <= .0000000000000000001):
        L = -y * (-7000000) - (1 - y) * math.log(1 - a)
    elif (a >= .999999999999):
        L = -y * math.log(a) - (1 - y) * (-7000000)
    else:
...
def predict(Xtest, w, b):
    ...
    for i, x in enumerate(Xtest):
        ...
        #########################################
        # INSERT YOUR CODE HERE
        # Compute Y, the predicted label matrix
        z = compute_z(x, w, b)
        a = compute_a(z)
        if a >= 0.5:
            Y[i] = 1
        ...
        # Compute the probability of instance having positive label
        P[i, 0] = a
        #########################################
    return Y, P
