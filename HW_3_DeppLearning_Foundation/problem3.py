...
def compute_a(z):
    ...
    # INSERT YOUR CODE HERE
    z_bound = 709.0
    if np.any(z > z_bound) or np.any(z < -z_bound):
        z = z.copy()
        z = z - z.max()             # Overflow
        z[z <= -z_bound] = -100     # Underflow
    z_exp = np.exp(z)
    a = z_exp / z_exp.sum()
    #########################################
    return a
...
def compute_dL_da(a, y):
    ...
    #########################################
    # INSERT YOUR CODE HERE
    dL_da = np.asmatrix(np.zeros((a.shape[0], 1)))
    if (a[y, 0] == 0):
        dL_da[y] = -1.0e10
    else:
        dL_da[y] = -1.0 / a[y, 0]
...
def compute_da_dz(a):
    ...
    # INSERT YOUR CODE HERE
    # Option 1
    da_dz = np.asmatrix(np.zeros((a.shape[0], a.shape[0])))
    for i in range(da_dz.shape[0]):
        for j in range(da_dz.shape[1]):
            if (i == j):
                da_dz[i, j] = a[i, 0] * (1 - a[i, 0])
            else:
                da_dz[i, j] = -a[i, 0] * a[j, 0]

    # Option 2
    # da_dz = -1.0 * np.matmul(a, a.T)
    # c = da_dz.shape[0]
    # for i in range(c):
...
def compute_dz_dW(x, c):
    ...
    #########################################
    # INSERT YOUR CODE HERE
    dz_dW = np.asmatrix(np.zeros((c, x.shape[0])))
    for i in range(c):
        dz_dW[i, :] = x.T
    #########################################
    return dz_dW
