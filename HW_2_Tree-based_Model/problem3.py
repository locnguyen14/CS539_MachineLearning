...
class Bag(DT):
    ...
    def bootstrap(X, Y):
        ...
        random_indexes = np.random.choice(a=X.shape[1], size=X.shape[1])
        X = X.T[random_indexes].T
        Y = Y[random_indexes]
    ...
    def inference(T, x):
        ...
        inferences = []
        for tree in T:
            inferences.append(DT.inference(tree, x))

        y = max(set(inferences), key=inferences.count)
        #########################################
        return y
    ...
    def predict(T, X):
        ...
        Y = np.empty(0)
        for instance in X.T:
            Y = np.append(Y, Bag.inference(T, instance))
        #########################################
        return Y.T
