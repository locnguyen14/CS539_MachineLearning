...
class Tree(object):
    ...
    def entropy(Y):
        ...
        # Create a Counter Object for list Y
        yCounter = Counter(Y)
        e = 0.0

        for value in yCounter:
            prob = yCounter[value] / sum(yCounter.values())
    ...
    def conditional_entropy(Y, X):
        ...
        for xValue in xCounter:
            # Probability for one value of X
            xProb = xCounter[xValue] / sum(xCounter.values())

            # Total Entropy of Y with respect to that X
            yEntropy = Tree.entropy(Y[np.where(X == xValue)])

            # Cummulative Conditional Entropy
            ce += xProb * yEntropy
    ...
    def best_attribute(X, Y):
        ...
        #########################################
        # INSERT YOUR CODE HERE
        # Compute a list of information gain (for each corresponding attribute)
        g_list = []
        for i in range(X.shape[0]):
            g = Tree.information_gain(Y, X[i, :])
            g_list.append(g)
        #########################################
        # Return the index of the attribute with highest information gain
        return np.argmax(g_list)
    ...
    def split(X, Y, i):
        ...
        #########################################
        # INSERT YOUR CODE HERE
        # Initialize the key list and the dictionary
        key_list = np.unique(X[i, :])
        C = {}

        for key in key_list:
            xChild = X.T[X[i, :] == key].T
            yChild = Y[X[i, :] == key]
            C[key] = Node(X=xChild, Y=yChild)
        #########################################
        return C
    ...
    def most_common(Y):
        ...
        #########################################
        # INSERT YOUR CODE HERE
        # Using Counter to store the class labels as list of tuple (label, number of appearance)
        yCounter = Counter(Y)
        y = yCounter.most_common(1)[0][0]
        #########################################
        return y
    ...
    def build_tree(t):
        ...
        # Assigning the most common label based on the class labels array in the node
        ...
        # Split the dataset based upon new best attribute
        t.C = Tree.split(t.X, t.Y, t.i)

        # recursively build subtree on each child node
        for nodeC in t.C:
            Tree.build_tree(t.C[nodeC])
        #########################################
        return
    #--------------------------
    ...
    def inference(t, x):
        ...
        #########################################
        # INSERT YOUR CODE HERE
        # infer the label recursively if the data t is not the leaf node
        while ((t.isleaf is False) and (x[t.i] in t.C.keys())):
            t = t.C[x[t.i]]
